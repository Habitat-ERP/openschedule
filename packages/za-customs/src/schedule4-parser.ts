...
import type {
  Schedule4PartV1,
  Schedule4ParsePageMetricsV1,
  Schedule4ParseResultV1,
  Schedule4RebateContextV1,
  Schedule4RebateLineV1
} from "./types.js";

export interface ParseSchedule4RebatesPdfOptions {
  pdfPath: string;
  pages?: readonly number[];
}

export interface ParseSchedule4RebatesTextPagesOptions {
...
export async function parseSchedule4RebatesPdf(options: ParseSchedule4RebatesPdfOptions): Promise<Schedule4ParseResultV1> {
  const extraction = await extractCustomsPdfTextItems(options);
  return parseSchedule4RebatesTextPages({
    pages: extraction.pages,
    sourceDocumentSha256: extraction.sourceDocumentSha256
  });
}

export function parseSchedule4RebatesTextPages(options: ParseSchedule4RebatesTextPagesOptions): Schedule4ParseResultV1 {
  const metrics = {
  ...
  const rebateLines: Schedule4RebateLineV1[] = [];
  const pageMetrics: Schedule4ParsePageMetricsV1[] = [];
  let activeContexts: Schedule4RebateContextV1[] = [];
  let activePart: Schedule4PartV1 = "unknown";

  for (const page of options.pages) {
    metrics.textItems += page.items.length;
    const pageRows = groupRows(page);
    metrics.layoutRows += pageRows.length;
    const currentPageMetrics: Schedule4ParsePageMetricsV1 = {
    ...
    for (const row of pageRows) {
      ...
      if (fields.rebateItem) {
        if (pending) {
          rebateLines.push(buildRebateLine(pending, options.sourceDocumentSha256, pageDate));
        ...
        if (isContextRow(row, layout, fields)) {
          const context = buildRebateContext(row, layout, fields, activePart, options.sourceDocumentSha256);
  ...
  if (!rebateLines.length) {
    warnings.push("No Schedule 4 rebate lines were parsed from the supplied pages.");
  }
...
function buildRebateLine(pending: PendingLine, sourceDocumentSha256: string, pageDate?: string): Schedule4RebateLineV1 {
  const warnings: string[] = [];
...
function isHeaderRow(row: LayoutRow): boolean {
  ...
  return /^(Date:|Rebate Item|Tariff Heading|Heading|Rebate Code|SCHEDULE 4|SPECIFIC REBATES|TEMPORARY REBATES|REBATES OF|IMPORTED GOODS|NOTES?:|\d+\.\s+)/.test(text);
}

function isContextRow(row: LayoutRow, layout: ColumnLayout, fields: RowFields): boolean {
  return Boolean(isSchedule4Item(fields.rebateItem) && !isRebateCandidate(fields) && readContextDescription(row, layout));
}

function isRebateCandidate(fields: RowFields): boolean {
  return Boolean(
    isSchedule4Item(fields.rebateItem) &&
...
function isSchedule4Item(value: string): boolean {
...
function extractPart(text: string): Schedule4PartV1 | null {
  const match = text.match(/\bSCHEDULE 4\s*(?:\/\s*)?PART\s*([1-6])\b/i);
  return match ? (match[1] as Schedule4PartV1) : null;
}
