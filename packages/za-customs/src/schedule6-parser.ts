...
import type {
  Schedule6ExciseRebateRefundContextV1,
  Schedule6ExciseRebateRefundLineV1,
  Schedule6ParsePageMetricsV1,
  Schedule6ParseResultV1
} from "./types.js";

export interface ParseSchedule6ExciseRebatesRefundsPdfOptions {
  pdfPath: string;
  pages?: readonly number[];
}

export interface ParseSchedule6ExciseRebatesRefundsTextPagesOptions {
...
export async function parseSchedule6ExciseRebatesRefundsPdf(
  options: ParseSchedule6ExciseRebatesRefundsPdfOptions
): Promise<Schedule6ParseResultV1> {
  const extraction = await extractCustomsPdfTextItems(options);
  return parseSchedule6ExciseRebatesRefundsTextPages({
    pages: extraction.pages,
    sourceDocumentSha256: extraction.sourceDocumentSha256
  });
}

export function parseSchedule6ExciseRebatesRefundsTextPages(
  options: ParseSchedule6ExciseRebatesRefundsTextPagesOptions
): Schedule6ParseResultV1 {
  const metrics = {
    ...
    candidateRows: 0,
    contextRows: 0,
    exciseRebateRefundLines: 0,
    rejectedRows: 0
  };
  const warnings: string[] = [];
  const exciseRebateRefundLines: Schedule6ExciseRebateRefundLineV1[] = [];
  const pageMetrics: Schedule6ParsePageMetricsV1[] = [];
  let activeContexts: Schedule6ExciseRebateRefundContextV1[] = [];
  let activePart = "unknown";

  for (const page of options.pages) {
    metrics.textItems += page.items.length;
    const pageRows = groupRows(page);
    metrics.layoutRows += pageRows.length;
    const currentPageMetrics: Schedule6ParsePageMetricsV1 = {
    ...
    for (const row of pageRows) {
      ...
      if (fields.item) {
        if (pending) {
          exciseRebateRefundLines.push(buildExciseRebateRefundLine(pending, options.sourceDocumentSha256, pageDate));
          metrics.exciseRebateRefundLines += 1;
          currentPageMetrics.exciseRebateRefundLines += 1;
        }
        pending = null;

        if (isContextRow(row, layout, fields)) {
          const context = buildExciseRebateRefundContext(row, layout, fields, activePart, options.sourceDocumentSha256);
        ...
        if (!isExciseRebateRefundCandidate(fields)) {
  ...
  if (!exciseRebateRefundLines.length) {
    warnings.push("No Schedule 6 excise rebate or refund lines were parsed from the supplied pages.");
  }
...
function detectColumnLayout(rows: LayoutRow[]): ColumnLayout {
  ...
  for (const row of rows.slice(0, 10)) {
    for (const item of row.items) {
      ...
      if (text === "Rebate Item" || text === "Refund Item" || text === "Item") layout.item = item.column;
      if (text === "Tariff Item" || text === "Tariff") layout.tariffItem = item.column;
      ...
      if (text === "Extent of Rebate") layout.extentOfRebate = item.column;
      if (text === "Extent of Refund") layout.extentOfRefund = item.column;
    }
  }
  return layout;
}
...
function readRowFields(row: LayoutRow, layout: ColumnLayout): RowFields {
  return {
    ...
    tariffItem: readColumn(row, layout, "tariffItem"),
    ...
    extentOfRefund: readColumn(row, layout, "extentOfRefund")
  };
}
...
function buildExciseRebateRefundLine(
  pending: PendingLine,
  sourceDocumentSha256: string,
  pageDate?: string
): Schedule6ExciseRebateRefundLineV1 {
  ...
  const rebateParts = [pending.fields.extentOfRebate];
  const refundParts = [pending.fields.extentOfRefund];

  for (const continuation of pending.continuationRows) {
    descriptionParts.push(continuation.fields.description);
    rebateParts.push(continuation.fields.extentOfRebate);
    refundParts.push(continuation.fields.extentOfRefund);
  }

  const description = compactJoin(descriptionParts);
  const extentOfRebate = compactJoin(rebateParts);
  const extentOfRefund = compactJoin(refundParts);

  if (!pending.fields.checkDigit) warnings.push("Missing check digit.");
  if (!description) warnings.push("Missing description.");
  if (!extentOfRebate && !extentOfRefund) warnings.push("Missing extent of rebate or refund.");
  ...
  return {
    ...
    tariffItem: pending.fields.tariffItem,
    normalizedTariffItem: normalizeDigits(pending.fields.tariffItem),
...
function buildExciseRebateRefundContext(
  row: LayoutRow,
  layout: ColumnLayout,
  fields: RowFields,
  part: string,
...
function isHeaderRow(row: LayoutRow): boolean {
  ...
  return /^(Date:|Rebate Item|Refund Item|Tariff Item|Rebate Code|Extent of|SCHEDULE 6|REBATES AND REFUNDS|SECTION |NOTES?:|\d+\.\s+)/.test(text);
}

function isContextRow(row: LayoutRow, layout: ColumnLayout, fields: RowFields): boolean {
  return Boolean(isSchedule6Item(fields.item) && !isExciseRebateRefundCandidate(fields) && readContextDescription(row, layout));
}

function isExciseRebateRefundCandidate(fields: RowFields): boolean {
  return Boolean(
    isSchedule6Item(fields.item) &&
      looksLikeDottedNumber(fields.tariffItem) &&
...
function isContinuationRow(fields: RowFields): boolean {
  return Boolean(!fields.item && (fields.description || fields.extentOfRebate || fields.extentOfRefund));
}

function isSchedule6Item(value: string): boolean {
...
function extractPart(text: string): string | null {
  const match = text.match(/\bSCHEDULE 6\s*(?:\/\s*)?PART\s*([0-9]+[A-Z]?)\b/i);
